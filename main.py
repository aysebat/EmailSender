...
def send_email_with_attachment_and_bill():
  """This function send to email to the people listed in csv file
The csv file containss the name and email combination, separated by commas. 
  """
  #read the csv file using pandas
  df = pd.read_csv("contacts_cp.csv")

  #iterate over all rows
  for index, row in df.iterrows():

    subject = "Bill for your purschase"

    #customize the contant with name from the csv file
    contents = [
      f"""
    Hello {row['name']}!
    Your payment is {row['bill']}
    ...
    print(f"Bill: {row['bill']}")
    print(f"File: {row['attacment']}")
    print('Email Send!')


send_email_with_attachment_and_bill()